...
class Courier:
    ...
    # метод авторизует курьера
    @staticmethod
    @allure.step('Авторизация курьера')
    def login_courier(registered_courier):
        del registered_courier[CourierService.EXCLUDE_PARAMETERS["firstName"]]
        response = requests.post(url=Endpoints.LOGIN_COURIER_URL, json=registered_courier)   # Выполнение POST-запроса
        return response
